...
create policy "audit_log_select_own" on public.audit_log
  for select using (
    rfp_id is null or exists (select 1 from public.rfps where rfps.id = audit_log.rfp_id and rfps.owner_id = auth.uid())
  );
...
create policy "users upload own files" on storage.objects
  for insert with check (
    bucket_id = 'rfp-files' and auth.uid() is not null
  );
